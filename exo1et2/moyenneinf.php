...
for ($i=0; $i <count($_SESSION['tableau']) ; $i++) { 
              ...
              if ($_SESSION['tableau'][$i]<$_SESSION['moyenne']) {
              	array_push($T['inferieur'], $_SESSION['tableau'][$i]) ;
              	}
            }

$page = ! empty( $_GET['page'] ) ? (int) $_GET['page'] : 1;
$total = count($T['inferieur']);  
$limit = 100; //par page    
...
//pagination($T['inferieur'],$li);

$T['inferieur']= array_slice( $T['inferieur'], $offset, $limit );

      echo ' <h2>l AFFICHAGE DU TABLEAU DES VALEURS INFERIEURS A LA MOYENNE : </h2> <br>';               
...
$s=0;
$nbt1=count($T['inferieur']);
for ($i=0; $i< $NbrLigne; $i++) {
   echo '<tr>';
       for ($j=0; $j<$NbrCol; $j++) {    
              echo '<td>';

	               if (!empty($T['inferieur'][$s])) {echo $T['inferieur'][$s];$s++;}
	                else
	                  {echo "null";}
              echo '</td>';
         }
